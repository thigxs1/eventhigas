...
function EventDetailPage() {
  ...
  async function uploadFlyer(file: File) {
    ...
    const path = `flyer-${eventId}-${Date.now()}.${ext}`;
    ...
    const { error } = await supabase.from("events").update({ flyer_url: data.publicUrl }).eq("id", eventId);
    if (error) {
    ...
    <div className="px-5 md:px-8 py-7 md:py-9 max-w-[1400px] mx-auto">
      ...
      <div className="rounded-2xl border border-border bg-[image:var(--gradient-surface)] p-6 mb-7">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          ...
          <div className="flex flex-col items-center gap-3 shrink-0 ml-auto">
            {event.flyer_url ? (
              <div className="relative group">
                <img 
                  src={event.flyer_url} 
                  alt="Flyer" 
                  className="size-28 rounded-xl object-cover border border-border shadow-sm transition-transform group-hover:scale-105"
                />
              ...
              <label className="size-28 rounded-xl border border-dashed border-border flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-surface/50 transition-colors text-muted-foreground group">
                <Upload className="size-5 group-hover:text-primary" />
                <span className="text-[10px] uppercase font-medium">Add Flyer</span>
                <input
                  type="file"
                  accept="image/*"
                  className="hidden"
            ...
            {event.flyer_url && (
              <Button 
                variant="ghost" 
                size="sm" 
                className="h-7 text-[10px] uppercase text-destructive hover:text-destructive hover:bg-destructive/10"
                onClick={removeFlyer}
              >
                Remover
              </Button>
...
function PublicSignupPanel({
  ...
  async function approveGuest(id: string) {
    const ticketType = selectedTypes[id] || (settings?.ticket_types?.[0] ?? "pista");
    const { error } = await supabase
      .from("guests")
      .update({ status: "approved", ticket_type: ticketType })
      .eq("id", id);
  ...
  async function approveAll() {
    ...
    const defaultType = settings?.ticket_types?.[0] ?? "pista";
    const { error } = await supabase
      .from("guests")
      .update({ status: "approved", ticket_type: defaultType })
      .in("id", pending.map((g) => g.id));
    if (error) toast.error("Erro ao aprovar todos");
    else {
      toast.success(`${pending.length} inscrições aprovadas com tipo '${defaultType}'`);
      onChanged();
    }
  }
              ...
              <tbody>
                {pending.map((g) => (
                  <tr key={g.id} className="border-t border-border hover:bg-surface/50">
                    ...
                    <td className="px-4 py-3">
                      <Select 
                        value={selectedTypes[g.id] || (settings?.ticket_types?.[0] ?? "pista")} 
                        onValueChange={(v) => setSelectedTypes(prev => ({ ...prev, [g.id]: v }))}
                      >
                        <SelectTrigger className="h-8 bg-surface">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          {(settings?.ticket_types ?? ["pista", "vip", "camarote"]).map(t => (
